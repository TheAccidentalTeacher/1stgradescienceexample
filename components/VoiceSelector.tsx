...
export default function VoiceSelector() {
  ...
  return (
    <div className="bg-gradient-to-r from-purple-100 via-blue-100 to-pink-100 rounded-2xl p-6 shadow-lg border-4 border-purple-300">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
        {voices.map((voice) => (
          <button
            ...
            disabled={isPlaying}
            className={`
              relative p-4 rounded-xl border-3 transition-all transform
              ${selectedVoice === voice.id 
                ? 'bg-purple-500 text-white border-purple-700 scale-105 shadow-xl' 
                : 'bg-white text-gray-800 border-gray-300 hover:border-purple-400 hover:shadow-md hover:scale-102'
              }
              ${isPlaying ? 'opacity-50 cursor-wait' : 'cursor-pointer'}
            `}
          >
            <div className="text-4xl mb-2">{voice.emoji}</div>
            <div className="text-xl font-bold mb-1">{voice.name}</div>
      ...
      <div className="bg-white rounded-xl p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="text-5xl">{currentVoice.emoji}</div>
          <div>
      ...
      {/* Tip */}
      <div className="mt-4 bg-yellow-100 border-2 border-yellow-400 rounded-xl p-3 text-center">
        <p className="text-sm text-yellow-900 font-semibold">
          💡 Tip: Click the blue speaker buttons <Volume2 className="inline w-4 h-4" /> anywhere in a lesson to hear that voice read to you!
        </p>
      </div>
    </div>
  )
